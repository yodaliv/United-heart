...
import { MasterAdminRegisterStepper } from './Steppers';

import { createOrganizationWithInvite } from '../../../../../apis/organization';
...
function MasterAdminRegister({ text, invite_code, STEP, step, setStep, progress, setProgress, history }) {
  ...
  const handleCreateOrganization = () => {
    ...
    createOrganizationWithInvite(params).then(res => {
      setLoading(false);
      setStep(step + 1);
      setProgress(progress + 1);
    }).catch(err => {
      setMessage({ open: true, title: text.error, description: text.failedCreateOrganization });
      setLoading(false);
    });
  };
  ...
  const handleContinue = () => {
    switch (step) {
      ...
      case STEP.FIVE:
        ...
        break;
      case STEP.SIX:
        if (values.org_name === '') setValidations(prevState => ({ ...prevState, org_name: 'has-empty' }));
        else if (values.address === '') setValidations(prevState => ({ ...prevState, address: 'has-empty' }));
        else if (values.city === '') setValidations(prevState => ({ ...prevState, city: 'has-empty' }));
        else if (values.state === '') setValidations(prevState => ({ ...prevState, state: 'has-empty' }));
        else if (values.postal === '') setValidations(prevState => ({ ...prevState, postal: 'has-empty' }));
        else if (values.country === '') setValidations(prevState => ({ ...prevState, country: 'has-empty' }));
        else handleCreateOrganization();
  ...
  return (
    <>
          ...
          <>
            ...
            <div className="mt-9 w-full">
              <TextInput
                ...
                onChange={val => handleChange('org_name', val)}
                error={validations.org_name !== ''}
                helperText={validations.org_name === 'has-empty' ? text.fieldRequired : ''}
              />
            </div>
            <div className="mt-9 w-full">
              <TextInput
                ...
                helperText={validations.address === 'has-empty' ? text.fieldRequired : ''}
              />
            </div>
            <div className="mt-6 w-full">
              <TextInput
                ...
                helperText={validations.city === 'has-empty' ? text.fieldRequired : ''}
              />
            </div>
            <div className="mt-6 w-full">
              <RegionSelect
                ...
                errorMessage={text.fieldRequired}
              />
            </div>
            <div className="mt-6 w-full">
              <TextInput
                label={text.postal}
                value={values.postal}
                onChange={val => handleChange('postal', val)}
                error={validations.postal !== ''}
                helperText={validations.postal === 'has-empty' ? text.fieldRequired : ''}
              />
            </div>
            <div className="mt-6 w-full">
              <CountrySelect
      ...
      <div className="mt-8">
        <MasterAdminRegisterStepper step={progress} />
      </div>
    </>
  );
}

export default MasterAdminRegister;
